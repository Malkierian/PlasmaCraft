...
public class CausticLakes
{
	...
	public static void populateSurfaceLiquids(World world, Random random, int x, int z)
	{
		rand = random;
        if(rand.nextFloat() >= 0.97f)
        {
            ...
            for(int i = 96; i > 50; i--)
            {
            	...
            	if(id != 0)
            	{
            		...
            		{
            			createSurfacePool(world, startX, i, startZ, rad, PlasmaCraftCore.acidStill.blockID, PlasmaCraftCore.acidMoving.blockID);
            			createSurfacePool(world, startX, i - 1, startZ, rad / 2, PlasmaCraftCore.acidStill.blockID, PlasmaCraftCore.acidMoving.blockID);
            			break;
            		}
        ...
        for(int k1 = 0; k1 < acidSpoutCount; k1++)
        {
            ...
            (new WorldGenCaustics(PlasmaCraftCore.acidMoving.blockID)).generate(world, rand, i3, k4, l5);
        }
            		...
            		|| biomegenbase == BiomeGenBase.taigaHills)
            {
            	for(int i = 96; i > 50; i--)
            	{
            		...
            		if(id != 0)
            		{
            			if(id == Block.grass.blockID)
            			{
            				createSurfacePool(world, startX, i, startZ, rad, PlasmaCraftCore.frozenCryonite.blockID, PlasmaCraftCore.frozenCryonite.blockID);
            				createSurfacePool(world, startX, i - 1, startZ, rad / 2, PlasmaCraftCore.frozenCryonite.blockID, PlasmaCraftCore.frozenCryonite.blockID);
                			break;
            			}
                		else if(id == Block.dirt.blockID || id == Block.waterStill.blockID ||
                				id == Block.stone.blockID || id == Block.cobblestone.blockID ||
	...
	public static void createSurfacePool(World world, int x, int y, int z, int rad, int stillId, int movingId)
	{
	...
	private static boolean bordersCaustic(World world, int x, int y, int z,
	...
	public static void populateNetherLiquids(World world, Random random, int x,	int z)
	{
		rand = random;
	}
	
	public static boolean isCaustic(World world, int x, int y, int z, int stillId, int movingId)
	{
		return (world.getBlockId(x, y, z) == stillId || world.getBlockId(x, y, z) == movingId);
	}
	
	public static void setWithChance(World world, int x, int y, int z, int stillId, int movingId, float chance, boolean force)
	{
	...
	private static boolean bordersAirOrLiquid(World world, int x, int y, int z)
	{
