...
label0:
        ...
        for(int k2 = 0; k2 < list.size(); k2++)
        {
            ...
            if(d4 <= 1.0D)
            {
                ...
                if(entity instanceof EntityLiving)
                {
                	...
                	el.addPotionEffect(new PotionEffect(PlasmaCraftCore.proxy.getPotionId(Potion.moveSlowdown), 10 * 20, 4));
                }
                double d14 = d13;
    ...
    public void doExplosionB()
    {
        ...
        for(int i = arraylist.size() - 1; i >= 0; i--)
        {
            ...
            else if(worldObj.isAirBlock(i1, l1, k2) && PlasmaCraftCore.frozenCryonite.canBlockStay(worldObj, i1, l1, k2))
            {
            	worldObj.setBlockWithNotify(PlasmaCraftCore.frozenCryonite.blockID, i1, l1, k2);
            }
        ...
        for(int k = (int)(posX - (double)(radius + 1.0F)); k <= (int)(posX + (double)(radius + 1.0F)); k++)
        {
            for(int l = (int)(posY - (double)(radius + 1.0F)); l <= (int)(posY + (double)(radius + 1.0F)); l++)
            {
                for(int k1 = (int)(posZ - (double)(radius + 1.0F)); k1 <= (int)(posZ + (double)(radius + 1.0F)); k1++)
                {
                    ...
                    if(j2 != PlasmaCraftCore.acidMoving.blockID && j2 != PlasmaCraftCore.acidStill.blockID)
                    {
                        continue;
                    }
