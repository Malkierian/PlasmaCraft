package untouchedwagons.minecraft.plasmacraft.worldgen;
		...
		if(rand.nextFloat() >= 0.97f)
		{
			...
			for(int i = 96; i > 50; i--)
			{
				...
				if(id != Blocks.air)
				{
					if(id == Blocks.grass)
					{
						createSurfacePool(world, startX, i, startZ, rad, PlasmaCraft.blocks.acidBlock);
						createSurfacePool(world, startX, i - 1, startZ, rad / 2, PlasmaCraft.blocks.acidBlock);
						break;
					}
					...
					|| biomegenbase == BiomeGenBase.taigaHills)
			{
				for(int i = 96; i > 50; i--)
				{
					...
					if(id != Blocks.air)
					{
						if(id == Blocks.grass)
						{
							createSurfacePool(world, startX, i, startZ, rad, PlasmaCraft.blocks.frozenCryonite);
							createSurfacePool(world, startX, i - 1, startZ, rad / 2, PlasmaCraft.blocks.frozenCryonite);
							break;
						}
