...
public class WorldGenerator implements IWorldGenerator {
	...
	private void generateNether(World world, Random random, int i, int j)
	{
		for(int k = 0; k < PlasmaCraft.neptuniumOreVeinCount; k++)
		{
			...
			(new WorldGenNetherMinable(PlasmaCraft.orePlasma, PlasmaCraft.neptuniumOreVeinSize, PlasmaCraft.neptuniumMeta)).generate(world, random, l1, i3, j4);
		}
		...
		for(int l = 0; l < PlasmaCraft.obsidiumOreVeinCount; l++)
		{
			...
			(new WorldGenNetherMinable(PlasmaCraft.orePlasma, PlasmaCraft.obsidiumOreVeinSize, PlasmaCraft.obsidiumMeta)).generate(world, random, i2, j3, k4);
		}
	...
	private void generateSurface(World world, Random random, int i, int j)
	{
		if(PlasmaCraft.generatePlutonium)
		{
			for(int k = 0; k < PlasmaCraft.plutoniumOreVeinCount; k++)
			{
				...
				(new WorldGenMulti(PlasmaCraft.orePlasma, PlasmaCraft.plutoniumOreVeinSize, PlasmaCraft.plutoniumMeta)).generate(world, random, i2, k3, i5);
			}
		}
		...
		if(PlasmaCraft.generateUranium)
		{
			for(int l = 0; l < PlasmaCraft.uraniumOreVeinCount; l++)
			{
				...
				(new WorldGenMulti(PlasmaCraft.orePlasma, PlasmaCraft.uraniumOreVeinSize, PlasmaCraft.uraniumMeta)).generate(world, random, j2, l3, j5);
			}
		}
		...
		for(int i1 = 0; i1 < PlasmaCraft.radioniteOreVeinCount; i1++)
		{
			...
			(new WorldGenMulti(PlasmaCraft.orePlasma, PlasmaCraft.radioniteOreVeinSize, PlasmaCraft.radioniteMeta)).generate(world, random, k2, i4, k5);
		}
