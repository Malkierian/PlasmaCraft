...
public class SMCryoBlast
{
    public SMCryoBlast(World world, Entity entity, double d, double d1, double d2, float f)
    {
        field_12257_a = false;
    ...
    public void doExplosionB()
    {
        ...
        for(int i = arraylist.size() - 1; i >= 0; i--)
        {
            ...
            if(i1 > 0)
            {
                ...
                {
               		if (worldObj.getBlockId(j, k, l) == Block.dirt.blockID || worldObj.getBlockId(j, k + 1, l) == Block.grass.blockID){
               			if (Block.fire.canPlaceBlockAt(worldObj, j, k, l) || !(worldObj.getBlockId(j, k - 1, l) == Block.snow.blockID)){
               				worldObj.setBlockWithNotify(j, k + 1, l, Block.snow.blockID);
               			}
               		}
                }
           		if (worldObj.getBlockId(j, k, l) == Block.waterStill.blockID || worldObj.getBlockId(j, k + 1, l) == Block.waterMoving.blockID){
           			worldObj.setBlockWithNotify(j, k, l, Block.ice.blockID);
           			worldObj.setBlockWithNotify(j, k + 1, l, Block.snow.blockID);
           		} else {
           			if (Block.fire.canPlaceBlockAt(worldObj, j, k, l) || !(worldObj.getBlockId(j, k - 1, l) == Block.snow.blockID)){
           				worldObj.setBlockWithNotify(j, k + 1, l, Block.snow.blockID);
