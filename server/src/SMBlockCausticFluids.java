...
public abstract class SMBlockCausticFluids extends Block
{
	...
	public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity)
    {
		...
		{
			world.setBlockWithNotify(i, j, k, mod_PlasmaCraft.oreUranium.blockID);
		}
    ...
    public void onEntityWalking(World world, int i,int j, int k, Entity entity)
    {
		...
		if(entity instanceof EntityPlayer || entity instanceof EntityPlayerMP)
		{
			...
			InventoryPlayer inv = new InventoryPlayer((EntityPlayer)entity);
			ItemStack helmetStack = inv.armorInventory[3];
			ItemStack plateStack = inv.armorInventory[2];
			ItemStack legsStack = inv.armorInventory[1];
			ItemStack bootsStack = inv.armorInventory[0];
    ...
    private void checkForHarden(World world, int i, int j, int k)
    {
		...
		if(blockIndexInTexture == mod_PlasmaCraft.acidStillIndex || blockIndexInTexture == mod_PlasmaCraft.acidMovingIndex)
		{
		 ...
		 if(blockAdjoinsBlockID(world, i, j, k, Block.lavaStill.blockID) || blockAdjoinsBlockID(world, i, j, k, Block.lavaMoving.blockID))
			...
			{
				if (mod_PlasmaCraft.allowCollisionExplosions == 1){
			...
			{
				setAdjoiningWaterIDs(world, i, j, k, Block.netherrack.blockID);
