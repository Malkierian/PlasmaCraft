...
public class EntityAcid extends Entity
{
	...
	public EntityAcid(World world, EntityLiving entityliving)
	{
		super(world);
		xTile = -1;
		yTile = -1;
		...
		if(movingobjectposition != null)
		{
			if(movingobjectposition.entityHit != null)
			{
				...
				if(movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, owner), 6))
				{
					...
					worldObj.setBlock(j, l, j1, PlasmaCraft.acidMoving.blockID, 0, 0);
					setDead();
				}
			} else
			{
				...
				{
					worldObj.setBlock(k, i1 + 1, k1, PlasmaCraft.acidMoving.blockID, 0, 0);
				}
				if(worldObj.isAirBlock(k, i1, k1 + 1) && flag)
				{
					worldObj.setBlock(k, i1, k1 + 1, PlasmaCraft.acidMoving.blockID, 0, 0);
				}
				if(worldObj.isAirBlock(k, i1, k1 - 1) && flag)
				{
					worldObj.setBlock(k, i1, k1 - 1, PlasmaCraft.acidMoving.blockID, 0, 0);
				...
				{
					worldObj.setBlock(k + 1, i1, k1, PlasmaCraft.acidMoving.blockID, 0, 0);
				}
				if(worldObj.isAirBlock(k - 1, i1, k1) && flag)
				{
					worldObj.setBlock(k - 1, i1, k1, PlasmaCraft.acidMoving.blockID, 0, 0);
